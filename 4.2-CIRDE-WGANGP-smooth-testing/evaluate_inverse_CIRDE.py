...
def save_individual_qualitative_images(
    ...
    """
    Save each qualitative reconstruction field as a separate image.

    For sample IDs 0, 1, 2, this produces

        src0.png, gen0.png, tar0.png
        src1.png, gen1.png, tar1.png
        src2.png, gen2.png, tar2.png

    The generated and target initial conditions use the same color scale
    so that they can be compared directly.
    """

    outdir.mkdir(parents=True, exist_ok=True)

    # Common color scale for generated and true initial conditions.
    ic_abs_max = max(
        np.max(np.abs(pred_scaled)),
        np.max(np.abs(true_scaled)),
    )

    # Separate scale for the terminal/source images.
    src_abs_max = np.max(np.abs(source_scaled))

    for i, sample_id in enumerate(sample_ids):

        # ------------------------------------------------------------
        # Given terminal state u_T
        # ------------------------------------------------------------
        plt.imsave(
            outdir / f"src{sample_id}.png",
            source_scaled[i, :, :, 0].T,
            cmap="magma",
            vmin=-src_abs_max,
            vmax=src_abs_max,
