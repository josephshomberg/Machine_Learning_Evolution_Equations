...
TRAIN_NPZ = "../../../tCHE_Datasets/Eyre_noisy_128x128-iterations=11000/train-dataset_128x128_tCH_PBC_Eyer_40-40-20_iters=11000_count=50000.npz"

TEST_NPZ  = "../../../tCHE_Datasets/Eyre_noisy_128x128-iterations=11000/test-dataset_128x128_tCH_PBC_Eyer_40-40-20_iters=11000_count=10000.npz"
...
# ============================================================
# Loss weights
# Eyre-data physics-informed WGAN
# Includes energy and short-horizon residual penalties
# ============================================================
...
MAX_STEPS      = 125000  #  =(dataset size)*(max epochs)/(batch size)
MAX_EPOCHS     = 10
BATCH_SIZE     = 4
VAL_BATCH_SIZE = 4
NUM_SAMPLES    = 8
...
if not os.path.exists(TEST_CSV):
    with open(TEST_CSV, mode="w", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "iteration", "samples", "mae", "energy_error", "mean_error", "variance_error",
            "mass_error", "purity_error", "gradient_error", "simplex_error",
            "residual_error", "gen_min", "gen_max", "gen_sum_min", "gen_sum_max",
            "gen_mean_c1", "gen_mean_c2", "gen_mean_c3", "gen_var_c1", 
            "gen_var_c2", "gen_var_c3",
        ])
...
def field_mean_loss_tf(real, fake):
    """Match component means across the batch (population-level statistic)."""
    mean_real = tf.reduce_mean(real, axis=[0, 1, 2])
    mean_fake = tf.reduce_mean(fake, axis=[0, 1, 2])
    return tf.reduce_mean(tf.abs(mean_real - mean_fake))


def mass_loss_tf(real, fake):
    """Match each sample's component masses (spatial means)."""
...
def summarize_performance(step, g_model, test_data, num_samples=NUM_SAMPLES, val_batch_size=VAL_BATCH_SIZE):
    ...
    for start in range(0, num_samples, val_batch_size):
        ...
        mean_err = field_mean_loss_tf(tar_tf, pred_tf).numpy()
        var_err = field_variance_loss_tf(tar_tf, pred_tf).numpy()
        ...
        variance_errors.append(float(var_err))
    ...
    print(
        ...
        f"mass={np.mean(mass_errors):.6f}, "
        f"purity={np.mean(purity_errors):.6f}, grad={np.mean(grad_errors):.6f}, "
        f"simplex={np.mean(simplex_errors):.6e}, residual={current_res:.6f}"
    )
...
def train_step(real_A, real_B, generator_model, critic_model, g_optimizer, c_optimizer):
    ...
    with tf.GradientTape() as tape:
        ...
        mean_loss = field_mean_loss_tf(real_B, fake_B)
    ...
    return (
    c_loss_accum, g_loss, adv_loss, mae_loss, energy_loss,
    mean_loss, var_loss, mass_loss, purity_loss, grad_loss, simplex_loss,
            residual_loss, entropy_loss, hf_loss,
    )
...
def train(c_model, g_model, wgan_model, dataset, max_epochs, n_batch=BATCH_SIZE):
    ...
    for i in range(start_i, min(n_steps, MAX_STEPS)):
        ...
        (
        c_loss, g_loss, adv_loss, mae_loss, energy_loss,
        mean_loss, var_loss, mass_loss, purity_loss, grad_loss, simplex_loss,
        residual_loss, entropy_loss, hf_loss
        ...
        if (i + 1) == 1 or (i + 1) % SAVEEVERY == 0:
            with open(TRAIN_CSV, mode="a", newline="") as f:
                ...
                w.writerow([
                    ...
                    float(mass_loss.numpy()), float(purity_loss.numpy()),
                    float(grad_loss.numpy()), float(simplex_loss.numpy()),
                    float(residual_loss.numpy()), float(entropy_loss.numpy()),
                    float(hf_loss.numpy()),
                ])
            try:
