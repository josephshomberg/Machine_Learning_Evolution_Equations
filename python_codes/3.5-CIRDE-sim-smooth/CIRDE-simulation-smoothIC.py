"""
2D Chafee--Infante equation with smooth Poisson initial data and
...
Time discretization is performed using an Eyre-style convex--concave
splitting scheme.  The linear diffusion term and cubic nonlinearity
are treated implicitly, while the expansive linear term is treated
...
initial_amplitude = 0.02  # amplitude scaling for smooth Poisson initial data
...
def smooth_dbc_ic_poisson(M, L2D, ell=0.05, amp=0.02, seed=None):
    """
    Generate a smooth initial condition with homogeneous Dirichlet
    boundary conditions via a screened Poisson solve:
        (I - ell^2 Delta) u = noise.

    The solve is performed on the interior grid only, and the boundary
    values are then set to zero.

    Parameters
    ----------
    M : int
        Total number of grid points per coordinate direction.
    L2D : scipy.sparse.csr_matrix
        Interior discrete Laplacian.
    ell : float, optional
        Correlation length parameter.
    amp : float, optional
        Amplitude scaling for the resulting field.
...
if os.path.exists(initial_data_path):
    ...
    print("Generating new Poisson initial condition...")
    phi[0] = smooth_dbc_ic_poisson(
        M,
        L2D,
        ell=0.06,
