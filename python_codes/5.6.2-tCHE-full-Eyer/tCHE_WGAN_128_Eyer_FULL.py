...
def field_mean_variance_loss_tf(real, fake):
...
@tf.function
def residual_loss_tf(c0_pred, cT_target, nsteps=RESIDUAL_STEPS):
    cT_sim = forward_sim_tche_tf(c0_pred, nsteps=nsteps)
    return tf.reduce_mean(tf.abs(cT_sim - cT_target))
...
def summarize_performance(step, g_model, test_data, num_samples=NUM_SAMPLES, val_batch_size=VAL_BATCH_SIZE):
    ...
    for start in range(0, num_samples, val_batch_size):
        ...
        if VALIDATE_RESIDUAL and residual_count < RESIDUAL_NUM_SAMPLES:
            ...
            res_err = residual_loss_tf(pred_tf[:take], src_tf[:take], nsteps=RESIDUAL_STEPS).numpy()
...
def train_step(real_A, real_B, generator_model, critic_model, g_optimizer, c_optimizer):
    ...
    with tf.GradientTape() as tape:
        ...
        mean_loss, var_loss = field_mean_variance_loss_tf(real_B, fake_B)
        ...
        if LAMBDA_RESIDUAL > 0.0:
            residual_loss = residual_loss_tf(fake_B, real_A, nsteps=RESIDUAL_STEPS)
        else:
