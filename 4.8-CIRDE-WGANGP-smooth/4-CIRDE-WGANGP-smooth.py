...
The generator loss combines
    ...
    3. short-horizon equal-time dynamical residual,
...
scheme. During neural-network training, the physics-informed loss uses a
differentiable Forward Euler surrogate over a short time horizon.

For

    tau = RESIDUAL_STEPS * DT,

the dynamical residual is

    R_tau(u0_pred, u0_true)
        = ||Phi_tau(u0_pred) - Phi_tau(u0_true)||_1.

Thus both the generated and true initial conditions are evolved for the
same short time interval. The residual measures local dynamical consistency
rather than attempting to reproduce the full dataset trajectory with the
        ...
        with TEST_CSV.open(
            ...
            writer.writerow(
                [
                    "iteration",
                    "mae_scaled",
                    "short_horizon_residual_scaled",
                    "mean_error_scaled",
                    "variance_error_scaled",
                    "energy_error_physical",
                    "mae_physical_IC",
                    "short_horizon_residual_physical",
...
# =============================================================================
# Physics: differentiable Forward Euler surrogate and short-horizon residual
# =============================================================================
...
def forward_consistency_loss(
    ...
    r"""
    Compute the short-horizon equal-time dynamical residual.

    Both the predicted and true initial conditions are evolved through
    the same differentiable Forward Euler surrogate for RESIDUAL_STEPS:
    ...
    The residual is normalized by the physical initial-condition scale.

    Parameters
    ----------
    u0_pred_scaled : tf.Tensor
        Predicted initial condition in scaled units, shape (B,M,M,1).
    u0_true_scaled : tf.Tensor
        True initial condition in scaled units, shape (B,M,M,1).
    ic_scale : float
        Physical scaling factor for initial conditions.
    reduce_batch : bool
        If True, return the batch mean. Otherwise return one residual per
        sample.

    Returns
    -------
    tf.Tensor
        Scalar batch mean if ``reduce_batch=True``; otherwise shape (B,).
    """
    ...
    per_sample_physical = tf.reduce_mean(
        tf.abs(
            pred_tau
            - true_tau
        ),
...
def summarize_performance(
    ...
    residual_scaled : float
        Mean scaled short-horizon equal-time dynamical residual.
    """
...
def main():
    ...
    print(
        "Physics residual:  "
        "equal-time short-horizon Forward Euler"
    )

    print(
        f"Residual horizon:   "
        f"{RESIDUAL_STEPS * DT:.6f} "
        f"({RESIDUAL_STEPS} steps)"
    )
