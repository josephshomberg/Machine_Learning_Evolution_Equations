...
# Equal-time physics residual horizon used by the WGAN.
RESIDUAL_STEPS = 100

TRAIN_NPZ       = '../../../../CIRDE_Datasets/128x128_kappa=4.7_DBC_Poisson_smooth_data_400iterations/train-dataset_128x128_DBC_kappa=4.7_PS400_count=50000.npz'

TEST_NPZ        = '../../../../CIRDE_Datasets/128x128_kappa=4.7_DBC_Poisson_smooth_data_400iterations/test-dataset_128x128_DBC_kappa=4.7_PS400_count=10000.npz'
...
BEST_MAE_MODEL_PATH = first_existing([
    os.path.join(MODEL_DIR, "best_generator_mae.keras"),
    os.path.join(MODEL_DIR, "best_generator.keras"),
    os.path.join(MODEL_DIR, "generator_mae.keras"),
    os.path.join(MODEL_DIR, "generator.keras"),
])

# If the MAE-only trainer used a different name, accept one unambiguous
# non-critic, non-residual generator file from training_output/models.
if BEST_MAE_MODEL_PATH is None and os.path.isdir(MODEL_DIR):
    mae_candidates = sorted(
        os.path.join(MODEL_DIR, filename)
        for filename in os.listdir(MODEL_DIR)
        if filename.endswith(".keras")
        and "generator" in filename.lower()
        and "critic" not in filename.lower()
        and "residual" not in filename.lower()
    )
    if len(mae_candidates) == 1:
        BEST_MAE_MODEL_PATH = mae_candidates[0]
...
BEST_MAE_METRIC_PATH = first_existing([
    os.path.join(MODEL_DIR, "best_mae.txt"),
    os.path.join(MODEL_DIR, "mae.txt"),
])

BEST_RES_MODEL_PATH = first_existing([
    os.path.join(MODEL_DIR, "best_generator_residual.keras"),
    os.path.join(MODEL_DIR, "generator_residual.keras"),
])

BEST_RES_METRIC_PATH = first_existing([
    os.path.join(MODEL_DIR, "best_residual.txt"),
    os.path.join(MODEL_DIR, "residual.txt"),
])

if BEST_MAE_MODEL_PATH is None:
    available = []
    if os.path.isdir(MODEL_DIR):
        available = sorted(os.listdir(MODEL_DIR))
    raise FileNotFoundError(
        "Could not locate an MAE generator in training_output/models. "
...
# A residual generator is genuinely optional.  The MAE-only evaluator still
# computes the residual horizon as a held-out diagnostic for best_mae.
if BEST_RES_MODEL_PATH is not None:
    MODEL_SPECS.append({
        "name": "best_residual",
...
class EnforceDirichletBoundary(Layer):
    """Set the complete boundary of every generator output equal to zero."""
...
def load_generator_model(model_path):
    ...
    return tf.keras.models.load_model(
        ...
        custom_objects={
            ...
            "MLEE>EnforceDirichletBoundary": EnforceDirichletBoundary,
        },
    )
...
def save_mae_residual_scatter(mae, residual, epsilon, delta, outpng):
    """Save the full-test MAE-versus-residual scatter plot."""
    fig, ax = plt.subplots(figsize=(7, 6))
    ax.scatter(mae, residual, s=10, alpha=0.45, edgecolors="none")
    ax.axvline(
        epsilon,
        color="tab:red",
        linestyle="--",
        linewidth=1.5,
        label=rf"$\varepsilon={epsilon:.6g}$",
    )
    ax.axhline(
        delta,
        color="tab:green",
        linestyle="--",
        linewidth=1.5,
        label=rf"$\delta={delta:.6g}$",
    )
    ax.set_xlabel("MAE (scaled IC units)")
    ax.set_ylabel("short-horizon residual (scaled IC units)")
    ax.set_title("MAE versus short-horizon residual")
    ax.grid(alpha=0.2)
    ax.legend()
    fig.tight_layout()
...
def save_qualitative_plot(
    ...
    for i in range(nrows):
        ...
        axes[i, 0].imshow(ne, cmap="magma", interpolation="none")

        axes[i, 1].imshow(gen, cmap="magma", interpolation="none")

        axes[i, 2].imshow(true, cmap="magma", interpolation="none")
        ...
        if save_individual:
            ...
            for name, img in zip(
                ...
                plt.imshow(img, cmap="magma", interpolation="none")
...
def forward_euler_horizon(u0_physical):
    """WGAN-matched Forward Euler surrogate on the interior DBC grid."""
    u = tf.cast(u0_physical[:, 1:-1, 1:-1, 0], tf.float32)
    h2 = tf.cast(h * h, tf.float32)
    dt_tf = tf.cast(dt, tf.float32)
    gamma_tf = tf.cast(gamma, tf.float32)
    kappa_tf = tf.cast(kappa, tf.float32)

    def condition(step, _u):
        return step < RESIDUAL_STEPS

    def body(step, u_current):
        padded = tf.pad(u_current, [[0, 0], [1, 1], [1, 1]])
        ...
        right = padded[:, 1:-1, 2:]
        laplacian = (
            (left - 2.0 * center + right) / h2
            + (up - 2.0 * center + down) / h2
        )
        reaction = -kappa_tf * (center**3 - center)
        u_next = center + dt_tf * (gamma_tf * laplacian + reaction)
        return step + 1, u_next

    _, u_tau = tf.while_loop(
        condition,
        body,
        (tf.constant(0, dtype=tf.int32), u),
...
def residual_per_sample(
    y_pred_scaled_np: np.ndarray,
    y_true_scaled_np: np.ndarray,
    ic_scale: float,
) -> np.ndarray:
    """Equal-time short-horizon residual in scaled initial-condition units.

    R_tau = mean|Phi_tau(IC_SCALE*y_pred)-Phi_tau(IC_SCALE*y_true)|/IC_SCALE,
    where tau = RESIDUAL_STEPS*dt.  The comparison is on the interior grid;
    both evolved fields have the same exact homogeneous Dirichlet boundary.
    """
    y_pred = tf.convert_to_tensor(y_pred_scaled_np, dtype=tf.float32)
    y_true = tf.convert_to_tensor(y_true_scaled_np, dtype=tf.float32)
    if y_pred.shape != y_true.shape:
        raise ValueError("Predicted and true initial-condition arrays must match.")
    if y_pred.shape[-1] != 1:
        raise ValueError("Expected one scalar channel.")

    scale = tf.cast(ic_scale, tf.float32)
    n_samples = int(y_pred.shape[0])
    residual_batches = []

    for start in range(0, n_samples, RESIDUAL_BATCH):
        end = min(n_samples, start + RESIDUAL_BATCH)
        pred_tau = forward_euler_horizon(scale * y_pred[start:end])
        true_tau = forward_euler_horizon(scale * y_true[start:end])
        residual_physical = tf.reduce_mean(
            tf.abs(pred_tau - true_tau), axis=[1, 2]
        )
        residual_batches.append((residual_physical / scale).numpy())

    return np.concatenate(residual_batches).astype(np.float64)
...
def evaluate_one_model(spec, X_test, Y_true, NE_SCALE, IC_SCALE):
    ...
    residual = residual_per_sample(Y_pred, Y_true, IC_SCALE)
    ...
    res_vals = residual_per_sample(x_gen_sam, x_true_sam, IC_SCALE)
    ...
    report = {
        ...
        "config": {
            ...
            "residual_horizon": RESIDUAL_STEPS * dt,

            "residual_definition": (
                "mean_abs(Phi_tau(IC_SCALE*y_pred) - "
                "Phi_tau(IC_SCALE*y_true)) / IC_SCALE"
            ),
    ...
    scatter_png = os.path.join(model_outdir, "mae_vs_residual.png")
    ...
    txt_lines.append("[Residual horizon]")

    txt_lines.append(f"  RESIDUAL_STEPS = {RESIDUAL_STEPS}")

    txt_lines.append(f"  tau            = {RESIDUAL_STEPS * dt:.6f}")

    txt_lines.append(
        "  definition     = mean_abs(Phi_tau(IC_SCALE*y_pred) - "
        "Phi_tau(IC_SCALE*y_true)) / IC_SCALE"
    )

    txt_lines.append("  comparison     = equal-time predicted and true IC rollouts")
...
def main():
    ...
    # Apply the exact homogeneous Dirichlet condition to the reference ICs.
    Y_true = enforce_dbc_numpy(Y_true)
